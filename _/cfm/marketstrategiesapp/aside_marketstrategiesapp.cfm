<section class="list-group css3-shadow">
    <a href="#" class="list-group-item active">Resources</a>

    <a href="#" class="list-group-item bg-gray">Brochures</a>
    <a href="/ag/_/pdf/marketstrategiesapp/ss_dtn_marketstrategies_app_2013.pdf" class="list-group-item" target="_blank"><i class="fa fa-file-pdf-o pad-rt"></i>DTN Market Strategies<sup>&reg;</sup> App</a>

<!---     <cfinclude template="/ag/_/cfm/marketstrategiesapp/snippet_marketstrategiesapp_cs.cfm">
 --->    <cfinclude template="/ag/_/cfm/marketstrategiesapp/snippet_marketstrategiesapp_vids.cfm"> 
	<cfinclude template="/ag/_/cfm/marketstrategiesapp/snippet_marketstrategiesapp_screens.cfm">
</section>